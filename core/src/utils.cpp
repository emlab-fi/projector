...
constexpr std::string_view op_type_map[] = {"nop", "join", "intersect", "cut"};

constexpr std::string_view shape_map[] = {"plane", "aa_box", "aa_ellipsoid",
                                          "aa_cylinder"};
...
void print_primitive(const projector::geom_primitive &prim, int level) {
    ...
    std::cout << prefix
              << "primitive: " << shape_map[static_cast<int>(prim.type)]
              << std::endl;
...
void print_operation(const projector::operation &op, int level) {
    ...
    std::cout << prefix << "operation: " << op_type_map[static_cast<int>(op.op)]
              << std::endl;
