...
bool read_file_header(std::ifstream &stream, uint32_t &atomic_number,
                      double &atomic_weight, std::size_t &energy_count) {
...
bool read_data_segment(std::ifstream &stream, std::vector<double> &data,
                       std::size_t count) {
...
element element::load_xcom_file(std::filesystem::path file) {
    ...
    if (!read_file_header(filestream, data.atomic_number, data.atomic_weight,
                          energy_count)) {
    ...
    for (std::size_t i = 1; i <= 100; ++i) {
        ...
        try {
            ...
            std::throw_with_nested(
                std::runtime_error("Error while reading XCOM file: " +
